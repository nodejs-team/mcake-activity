...
(function($) {
	...
	$.fn.barrager = function(barrage) {
		...
		var this_width =  (window_width > this.width()) ? this.width()+850 : window_width;

		if(this_width<=0){
      		this_width= window_width-100
		}


		var bottom = (barrage.bottom == 0) ? Math.floor(Math.random() * this_height + 40) : barrage.bottom;
		div_barrager.css("bottom", px2rem(bottom) + "rem");
		...
		div_barrager.css('margin-right', 0);
 		...
 		$(id).animate({right:px2rem(this_width) + "rem"},barrage.speed*1000,function(){
			$(id).remove();
		});
...
*/
     ...
     console.log(barrage.speed*1000)
