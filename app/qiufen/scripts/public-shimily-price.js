...
;(function($){
    ...
    Price.prototype={
        ...
        counts:function (ele) {
            ...
            /*判断是否换购wap*/
            var hasOn = ele.parents(".price").find('.icon').hasClass("on");
            if(hasOn){
                var sku = ele.parents(".price").find('.huangou').data("sku"); /*换购sku*/
                var cakeSku =  ele.data('sku');
                ele.parents(".price").find(".go-buy").attr("data-sku",cakeSku+","+sku);
                ele.parents(".price").find(".go-buy").attr("data-num",self.num+","+1);
            }
