  ...
  function initNum() {
    ...
    $(".products li").each(function(i,el){
      ...
      /*磅数按钮形式切换*/
        ...
        var len = items[index].ponds.length;
        var str = '';
        for(var i=0; i<len; i++){

            str = '<dd data-oldPrice="198" data-bs="1.5" data-weight="618g"><div class="ok"></div><span class="bs">'+items[index].ponds[i]+'</span></dd>';
            $(this).find(".select-pond ol").append(str);
            $(this).find(".select-pond ol dd").eq(0).addClass("cur");
        }


        $(this).find(".select-pond dd").on('click', function(){
            currentItem.ix = $(this).index();
            var ix = currentItem.ix;
      ...
      /*初始化*/
        ...
        /*默认2磅，排除只有一个磅数的情况*/
      /*if(currentItem.ponds.length==1) {  /!*只有一个磅数*!/
        /!*是否有折扣*!/
          totalPrice = currentItem.ids[0]; /!*总价格*!/
          oldtotalPrice = totalPrice-0;
          totalPrice = isDiscount(totalPrice,1,discount,zhekou); /!*满足条件折扣*!/
        ...
        /!*设置默认显示2磅*!/
          ...
          oldtotalPrice = totalPrice-0;
          totalPrice = isDiscount(totalPrice,1,discount,zhekou); /!*满足条件折扣*!/
          $(this).find(".bang").html(currentItem.ponds[1]);
          $(this).find(".dis-price").html(totalPrice.toFixed(2));
