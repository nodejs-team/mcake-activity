...
function importCom() {
	...
	com.androidKeyboard = function(option) {
        option = option || {}
        option.scrollbox = option.scrollbox || window
        option.type = option.type || 'scroll'
        option.space = option.space || 50
        if (os.android) {
          var h = $(window).height()
          var sto = {
            scroll: function(y) {option.scrollbox.scrollTop(y)},
            transform: function(y) {$(option.scrollbox).css({ transform: 'translate(0, ' + -y + 'px)' })}
          }
          var sy = {}
          $('input', option.scrollbox).off('focus.androidKeyboard').on('focus.androidKeyboard', function() {
              var that = this
              setTimeout(function() {
				var hh = $(window).height();
				sy.scroll = $(option.scrollbox).scrollTop()
            	sy.transform =-parseInt($(option.scrollbox).css('transform').split(',')[5]) || 0
                //键盘弹出后判断当前input是否在小屏幕区域中，在就不作处理，不在就scroll
                if ($(that).offset().top + that.clientHeight > hh) {
				  var s =$(that).offset().top +sy[option.type] -hh +that.clientHeight +option.space;
				  sto[option.type](s)
                }
              }, 500)
            })
          $(window).off('resize.androidKeyboard').on('resize.androidKeyboard', function() {
              var hh = $(window).height()
              if (hh == h) {
                $('input').blur();
								// option.scrollbox.scrollTop(sy[option.type])
