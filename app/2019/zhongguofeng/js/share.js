...
(function () {
	...
	ishare.content = {
		link: ishare.url,
		image: ishare.url + 'resource/share.jpg?v=' + ((Math.random() * 10000) >> 0).toString(),
...
function importShare() {
	...
	share.wxSign = function () {
		...
		function setHttpsApi() {
			$.get("https://scrm.beats-digital.com/api/jssdk/sign", { appid: share.wxId, url: location.href }, function (data) {
				ApiConfig(data)
			}, 'json');//end ajax
		}
